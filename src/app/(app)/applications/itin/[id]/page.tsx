...
const ITIN_STATUSES: Record<ItinStatus, { label: string; color: string; icon: "check" | "clock" | "x" }> = {
  ...
  CAA_SCHEDULED:   { label: "CAA appointment set",   color: "bg-blue-100 text-blue-700",         icon: "clock" },
  W7_SUBMITTED:    { label: "W-7 submitted to IRS",  color: "bg-blue-100 text-blue-700",         icon: "clock" },
  COMPLETED:       { label: "Completed — ITIN issued", color: "bg-emerald-100 text-emerald-700", icon: "check" },
  CANCELLED:       { label: "Cancelled",             color: "bg-red-100 text-red-700",           icon: "x" },
};

export default async function ItinApplicationPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">ITIN Application</h1>
          <p className="text-slate-500 mt-1">{app.fullName}</p>
        </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white divide-y divide-slate-100">
        <div className="px-5 py-4">
          ...
          <dl className="space-y-2 text-sm">
            <div className="flex"><dt className="w-40 text-slate-500 shrink-0">Name</dt><dd className="text-slate-900">{app.fullName}</dd></div>
            <div className="flex"><dt className="w-40 text-slate-500 shrink-0">W-7 reason</dt><dd className="text-slate-900">{app.itinReason}</dd></div>
            {app.usActivity && <div className="flex"><dt className="w-40 text-slate-500 shrink-0">US activity</dt><dd className="text-slate-900">{app.usActivity}</dd></div>}
      ...
      <div className="mt-6">
        <MessagesPanel apiBase={`/api/applications/itin/${app.id}/messages`} isAdmin={false} />
      </div>
