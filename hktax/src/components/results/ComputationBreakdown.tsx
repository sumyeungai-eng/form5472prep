  ...
  return (
    <section className={`results-breakdown-section rounded-lg border border-warm-200 bg-white p-5 shadow-soft sm:p-8${printHidden ? " results-breakdown-baseline" : ""}`}>
      <div>
        ...
        <h2 className="mt-1 text-xl font-bold text-navy-900">{heading}</h2>
      </div>
...
function ComputationPanel({
  ...
  return (
    <details open={defaultOpen || undefined} className="rounded-md border border-warm-200 bg-warm-50">
      <summary className="focus-ring flex cursor-pointer list-none items-center justify-between gap-4 rounded-md px-4 py-3 text-left text-sm font-bold text-navy-900">
      ...
      <div className="border-t border-warm-200 bg-white p-4">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full divide-y divide-warm-200 text-sm">
            <thead className="bg-warm-50 text-left text-xs uppercase text-warm-600">
              <tr>
...
function SummaryMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-warm-100 bg-white p-3">
