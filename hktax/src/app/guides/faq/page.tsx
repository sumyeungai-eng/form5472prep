...
const SIDE_BUSINESS_PLACEHOLDER_ZH =
  "這屬於獨資及合夥業務利得稅的「兩級制」：首 {profitsTierOneCap} 應評稅利潤按 {profitsTierOneRate} 課稅，超出部分按 {profitsStandardRate} 課稅。要留意的是，如你或與你有關連的其他實體已選用兩級制，同一組有關連實體一般只可由其中一個享用兩級制稅率，你須在報稅時申報是否符合資格。";
...
const MPF_PLACEHOLDER_ZH =
  "強制性供款是法例要求的最低供款，其相關的個人扣除設有每年 {mpfMandatoryCap} 上限。TVC（可扣稅自願性供款）則是額外的自願性供款，與合資格年金保費共用另一個每年 {annuityAndTvcCap} 的合併上限；換言之，TVC 與強制性供款屬於兩個獨立的扣除額度，不會互相佔用對方的上限。";
...
const NEWBORN_PLACEHOLDER_ZH =
  "在子女出生的課稅年度，除每名子女 {childAllowance} 的子女免稅額之外，另可獲得一次性 {childNewbornExtra} 的額外子女免稅額。如你符合條件，並與在 2023 年 10 月 25 日或之後出生的首名子女同住，居所貸款利息扣除及住宅租金扣除的上限亦會由 {homeLoanInterestCap} / {domesticRentCap} 提高至 {homeLoanInterestElevatedCap} / {domesticRentElevatedCap}。";
...
const PROPERTY_REDUCTION_PLACEHOLDER_ZH =
  "一般而言，物業稅並不在稅務局每年公布的一次性稅務寬減範圍之內；寬減通常只適用於薪俸稅、利得稅及個人入息課稅（稅款的 {reductionPercent}，並設每宗個案上限）。如你只有物業出租收入並以物業稅方式課稅，多數不會享有該項寬減；如你改為選擇個人入息課稅，則有機會納入寬減範圍。實際適用範圍請以稅務局每年的最新公布為準。";
...
const faqItems: FaqItem[] = [
  {
    ...
    answerZh:
      "先確認你有沒有收到稅務局的個別人士報稅表（BIR60）；如未收到但已有應課稅入息，你須主動通知稅務局（見〈報稅時間表〉指南）。建議先用本網站的「報稅精靈」，按你的實際入息、扣除及家庭狀況試算一次，了解大概稅款及須填報的部分，再對照正式報稅表逐項填寫。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "已婚人士的薪俸稅一般為「分開評稅」，但夫婦亦可選擇「合併評稅」，將雙方入息合併計算並使用已婚人士免稅額；由 2018/19 課稅年度起，已婚人士亦可個別選擇「個人入息課稅」，或在雙方同意下共同選擇。哪一種組合最節省稅款，視乎雙方入息差距、扣除項目及是否有虧損等因素而定，本網站的優化器會列出各方案的總稅款作比較。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "不可以：兩者不能同時申索，在同一課稅年度只可選擇其中一項。此外，如你或配偶在香港擁有任何已評估應課差餉租值的住宅物業，一般不符合住宅租金扣除的資格，即使該物業並非你現時居住的單位亦然；這也是稅務局經常查核的重點。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "如你就該出租物業支付按揭利息，物業稅本身並不容許扣除該筆利息，但個人入息課稅則容許扣除（上限為該物業的應評稅淨值）。如你同時有其他入息（例如薪俸），個人入息課稅會將所有入息合併計算，是否更節省稅款須視乎整體數字，建議用本網站的優化器逐一比較。",
    answerEn:
  ...
  },
  {
    ...
    questionZh: "我有一項副業，利潤未達兩級制門檻，稅率如何計算？",
  ...
  {
    id: "newborn-caps",
    questionZh: "子女出生的課稅年度，有甚麼額外稅務優惠？",
  ...
  {
    id: "share-options",
    questionZh: "僱主授予我股份認購權（share options），應如何計稅？",
    questionEn: "My employer granted me share options — how are they taxed?",
    answerZh:
      "行使、轉讓或放棄股份認購權時所得的收益，一般會視為應課稅的僱傭入息，須計入行使或轉讓當年的薪俸稅評稅內。計算方法，以及涉及跨境受僱、分期歸屬（vesting）等情況，均可以相當複雜；本網站並未涵蓋股份認購權的自動計算，建議交由執業會計師或稅務顧問處理。",
    answerEn:
      "Gains realised when you exercise, assign, or release a share option are generally treated as taxable employment income assessable in the year of exercise or assignment. The computation — especially where employment spans multiple jurisdictions or vesting is staggered — can be complex. This site does not automate share option computations; consult a qualified accountant or tax adviser for your situation."
  },
  {
    id: "leaving-hk",
    ...
    answerZh:
      "一般而言，僱主須在你預計離職日前最少一個月，以書面通知稅務局，並可能須暫扣你的最後薪金，直至取得稅務局發出的「同意釋款書」（俗稱「清稅」）為止。你本人亦應主動通知稅務局你即將離港，並如實申報全年入息；所有稅款清繳後，被暫扣的款項才會發放。詳情及所需表格請以稅務局最新指引為準。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "概括而言：如夫婦入息相差懸殊，或其中一方的大額扣除項目（例如按揭利息、業務虧損）超過其本人的入息、未能全數扣減，則合併評稅或個人入息課稅通常較有利，因為可將未用完的扣除額轉去抵銷另一方的入息。如雙方入息相若、各自已用盡扣除及免稅額，分開評稅未必較差。實際結果因人而異，建議一律以本網站優化器的逐項比較為準，不要單憑經驗法則作出決定。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "建議保留所有支持你申報數字的文件，包括僱主發出的薪酬報稅表（IR56 系列）、扣除項目的收據（自願醫保保費單、進修學費單、慈善捐款收據等）、租約及租金收據、按揭利息證明，以及物業或業務的帳目紀錄。法例對不同類別的紀錄設有不同的保存年期規定，具體年期請參閱稅務局最新指引，不宜自行假設。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "eTAX 網上報稅通常可享有較長的申報限期，遞交後即時獲得確認，亦可查閱過往的申報紀錄，並省卻郵遞所需時間；紙本申報則須親筆簽署，並以郵寄或親身遞交方式交回，限期一般較短。兩者所須申報的資料內容大致相同，選擇哪一種主要視乎你是否已登記 eTAX 帳戶及個人使用習慣。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "不會。本網站的所有計算均在你的瀏覽器內完成，你輸入的入息、家庭及扣除資料不會傳送到任何伺服器，我們亦沒有帳戶系統或後台資料庫。如你選擇讓報稅精靈記住你的填寫進度，資料只會儲存在你自己裝置的瀏覽器（localStorage）內，你可以隨時按「清除我的資料」刪除。",
    answerEn:
  ...
  },
  {
    ...
    questionZh: "我不是香港居民，只在香港工作了數十天，是否須繳付薪俸稅？",
    questionEn: "I'm not a Hong Kong resident and only worked here for a number of days — do I owe salaries tax?",
    answerZh:
      "非香港居民的短期受僱情況，涉及「60 天規則」及按在港工作日數比例劃分入息等複雜判斷，須逐個個案考慮。本網站目前並未提供自動計算，此處僅作提示。如你屬於這類情況，強烈建議諮詢執業稅務顧問，確認實際的申報及繳稅責任。",
    answerEn:
  ...
  },
  {
    ...
    answerZh:
      "所有金額及稅率均取自稅務局官方發布的《免稅額、扣除及稅率表》（PAM 61(e)）及相關 ird.gov.hk / gov.hk 官方頁面，並按課稅年度分別存放在網站的參數檔案內，逐項核對來源後才使用；本網站的文字內容不會直接寫入任何固定的稅務金額，所有數字都是即時按已核實的參數計算出來。即使如此，本網站仍屬教育及估算工具，正式數字請以稅務局發出的通知為準。",
    answerEn:
      ...
      <section className="bg-white py-14 sm:py-16">
        <Container>
          <p className="max-w-3xl text-xs leading-6 text-warm-600">
            {lang === "zh"
              ? "本頁內容僅供一般教育及參考用途，並非稅務意見；本網站與稅務局並無從屬關係。個別情況可能有例外或額外規定，實際安排請以稅務局最新指引及你本人收到的通知為準；如有疑問，建議諮詢執業會計師或稅務顧問。"
